...
Todo o desenvolvimento do projeto será realizado no [Glitch](htmlbasico.qmd#disponibilizando-páginas-web-diretamente-na-internet-com-glitch). Vide @sec-glitch.

(@) Faça o login na sua conta do Glitch e remixe o projeto [aframe-minicurso-vr](https://glitch.com/edit/#!/aframe-minicurso-vr)
...
(@) O elemento HTML básico e obrigatório do A-Frame é o `a-scene` ([Cena](https://aframe.io/docs/1.6.0/core/scene.html)), o qual temos que incluir dentro do elemento `body` conforme o @cod-scene.
...
```
O elemento básico, `a-scene`, de uma página HTML com A-Frame.
:::
...
![Ilustração das *Entidades* carro e moto com seus *componentes* e *atributos* específicos (feita com DALL-E)](carro_e_moto.jpg){#fig-fuscamoto}
...
(@) Coloque a entidade do @cod-terra dentro do elemento a-scene do @cod-scene.
...
```
Início da criação do planeta Terra -- uma esfera azul por enquanto. 
:::

Se fizermos o preview da cena no Glitch poderemos observar a esfera de diferentes ângulos usando o mouse. Mas não conseguiremos dar zoom ou deslocamentos laterais na imagem. Isso ocorre pois não estamos usando os óculos de realidade virtual. A melhor forma de interagir com a página gerada na tela do computador, sem os óculos, é escolhendo a opção *Preview in a new window* no Glitch. Na página que foi aberta, devemos entrar no modo de inspeção ([Visual inspector](https://aframe.io/docs/1.6.0/introduction/visual-inspector-and-dev-tools.html#a-frame-inspector)) ao digitar a combinação de teclas `<ctrl>` + `<alt>` + `i`.
...
![[Mapa terrestre em projeção cilíndrica (*equirectangular*). Fonte: Wikimedia](https://upload.wikimedia.org/wikipedia/commons/thumb/2/23/Blue_Marble_2002.png/1280px-Blue_Marble_2002.png)](1280px-Blue_Marble_2002.png)

(@) Dentro do elemento `a-scene` inclua o elemento `img` conform e o @cod-asset.
...
``` 
Inclusão de imagens e outros recursos via elemento `a-asset`.
:::

(@) Em seguida substitua o atributo *color* por *src* no componente *material* da entidade `terra`, usando como valor a `id` do mapa da Terra, resultando no @cod-terra-com-mapa
...
``` 
Entidade `terra`com atributo `src` no lugar de `color`.
:::
...
(@) Primeiramente, vamos atualizar os nossos assets com as imagens das superfícies do Sol e da Lua conforme o @cod-assets-sol-terra-lua.
    
::: {#cod-assets-sol-terra-lua}
```html
<a-assets>
...
```
Imagens dos mapas em projeção cilíndrica de todos os astros do projeto incluídos como assets.
:::
...
Agora, vamos colocar a entidade que representa a esfera de cada astro dentro dos respectivos referenciais. Para isso, substitua o @cod-terra-com-mapa pelo @cod-astros-com-mapa.
...
```
Todos os astros com seus respectivos mapas de superfície aninhado em seus respectivos referenciais (entidades).
:::

Repare que para o raio esférico de cada astro usamos o atributo `radius` do componente `geometry`. Outra observação é que todas as dimensões são informadas ao A-Frame sem explicitar as unidades, pois o A-Frame considera todas elas em metros por padrão. A @fig-geometria mostra a disposição geométrica do sistema.

![Sol-Terra-Lua gerado pelo @cod-astros-com-mapa e visto de cima. Cada divisão do grid (em cinza claro) corresponde a 0,5 metros. As setas correspondem às direções x (vermelha à direita), y (verde saindo da tela) e z (azul para baixo).](STL_visto_de_cima.png){#fig-geometria}


## Animando as coisas

Como diria o sábio Galileu, "*Eppur si muove*".[^2] Vamos então pôr o mundo a girar.
...
Com ele, deveremos observar a Terra orbitando em torno do Sol *ad aeternum* (`loop: true`) com velocidade constante (`easing: linear`) e executando uma revolução (`to: 0 360 0`) a cada 300 segundos (`dur: 300000`) conforme @vid-orbita-terra-travada.

![Ilustração do movimento orbital da Terra usando o `animation` da `a-entity orb_terra`. A cada órbita completa a Terra dá, também, uma volta em torno do próprio eixo (quando próxima à câmera apresenta a África voltada para nós, quando longe apresenta o Pacífico). (10x faster)](movimento_orbital_travado.mp4){#vid-orbita-terra-travada loop="true"}
...
Esse código resulta no movimento orbital da Terra sem o movimento espúrio de rotação em torno do próprio eixo, conforme vemos no @vid-orbita-terra-corrigida.

![Ilustração do movimento orbital da Terra com a compensação da rotação espúria em torno do próprio eixo usando um `animation` com rotação no sentido contrário `dir:reverse`. Nesse caso, a Terra não gira em torno do próprio eixo, estando as Américas sempre voltadas para a câmera. (10x faster)](movimento_orbital_livre.mp4){#vid-orbita-terra-corrigida loop="true"}
...
Assim, deveremos observar a Terra girando *ad aeternum* (`loop: true`) com velocidade constante (`easing: linear`) e executando uma revolução (`to: 0 360 0`) a cada 10 segundos (`dur: 10000`). O @vid-terra-rotacao mostra esse movimento isoladamente (sem o movimento orbital).

![Terra em movimento de rotação com o componente `animation`.](terra_girando.mp4){#vid-terra-rotacao loop="true"}


### Movimento completo

(@) Implementado os movimentos de rotação e orbital discutidos acima no sistema Sol-Terra-Lua completo temos o @cod-stl-orbital que substitui o @cod-astros-com-mapa
...
```
Sistema STL com movimento orbital e de rotação da Terra.
:::
...
![Sistema Sol-Terra-Lua com todos os movimentos -- de rotação e orbital -- implementados, visto de cima. (10x faster)](stl_animado.mp4){#vid-terra-orbitando loop="true"}
...
Com isso temos todos os movimentos desejados para as nossas finalidades didáticas conforme mostrado no @vid-ovimento-completo-com-inclinacoes.

![Sistema Sol-Terra-Lua com todos os movimentos e inclinações do eixo da Terra e da órbita da Lua, visto de perfil. (10x faster)](movimento-completo-com-inclinacoes.mp4){#vid-ovimento-completo-com-inclinacoes loop="true"}
...
Essa imagem (@fig-skymap) foi obtida do [catálogo estelar Tycho](https://svs.gsfc.nasa.gov/3442/) e colocada temporariamente no *postimg* para que pudesse ser carregada online pelo a-Frame.

![Mapa estelar com estrelas dos catálogos [Tycho e Hipparcos](https://svs.gsfc.nasa.gov/3442/), em projeção cilíndrica equidistante (plate carrée) utilizando coordenadas celestes. Adequado para mapeamento em esferas em programas de animação. As estrelas são representadas como *Point Spread Functions* (PSF), com tamanho e intensidade correspondentes à sua luminosidade relativa.](TychoSkymap.t4_04096x02048.jpg){#fig-skymap}

## Resultado final

::: {#cod-stl-completo}
::: {.callout-tip title="Clique para expandir" collapse="true"}
```html
<!DOCTYPE html>
...
```
:::
Código completo do sistema STL.
:::

::: {#vid-stl-completo}
{{< video https://youtu.be/Xs07RB4W-kc >}}

Sistema STL em movimento com câmera estática externa.
:::

Veja também o [sistema STL completo rodando no Glitch](https://stl-completo.glitch.me).
...
No código acima, aumentamos a segmentação da esfera terrestre para uma superfície mais suave e aumentamos o seu raio para um horizonte menos curvo. Fora isso, o código é essencialmente o mesmo do @cod-stl-completo.

Essa posição da câmera produz a visualização mostrada no @vid-stl-na-terra.

::: {#vid-stl-na-terra}
{{< video https://youtu.be/J6ZFjDjrnjc >}}
Sistema STL visto por uma câmera colocada na superfície da Terra.
:::

Veja também o [sistema Sol-Terra-Lua com câmera fixa na superfície da Terra no Glitch](https://stl-camera-terra.glitch.me).
...
O @cod-stl-grabbable corresponde a um sistema STL alternativo mais simples em que os astros não são animados, mas apenas habilitados para serem "agarráveis" pelo usuário. Apesar dessa simplicidade, as condições de iluminação dentro desse ambiente virtual são difíceis de reproduzir em um ambiente real, possibilitando explorar com riqueza os fenômenos que mencionamos anteriormente envolvendo as fases da Lua, eclipses e estações do ano. Vide @vid-stl-grabbable.
...
```
Sistema STL simplificado sem animação, mas que permite que os objetos sejam agarráveis pelas mãos do usuário.
:::

::: {#vid-stl-grabbable}
{{< video https://youtu.be/v-wMky5iR70 >}}

Vídeo mostrando a manipulação e os usos didáticos do sistema STL simplificado
:::

Veja também o [sistema Sol-Terra-Lua manipulável no Glitch](https://stl-grabbable.glitch.me).
...
Para isso, são necessários dois passos. Primeiramente, deve-se incluir o script Javascript (@cod-script-offline) dentro do elemento `body` do arquivo html principal.
...
Por fim, deve-se criar um arquivo como o nome `service-worker.js` na mesma pasta do arquivo html principal com o com o conteúdo do @cod-service-worker.
...
Conteúdo do arquivo `service-worker.js` para acesso offline da aplicação web. Aqui estamos considerando que o arquivo principal chama-se `index.html`.
:::
...
Com isso, concluímos este minicurso. Apesar do A-Frame permitir a programação usando exclusivamente linguagem HTML, é possível usar todo o poder da linguagem Javascript para desenvolver aplicações mais sofisticadas. O modo correto de fazer isso no A-Frame é através da [criação de novos *Components*](https://aframe.io/docs/1.6.0/introduction/writing-a-component.html), os quais receberão toda a lógica de programação em Javascript. Além disso, o A-Frame disponibiliza acesso a todas as funcionalidades da poderosa biblioteca gráfica [Three.js](https://threejs.org/) (pois é uma abstração desta). E existe também as [componentes criadas pela comunidade de desenvolvedores](https://aframe.wiki/en/#!pages/component-directory.md), as quais abrangem uma boa diversidade de usos. Escolhemos não abordar essas possibilidades devido à curta duração deste minicurso.
