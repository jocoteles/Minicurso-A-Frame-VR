...
Assim, deveremos observar a Terra girando *ad eternum* (`loop: true`) com velocidade constante (`easing: linear`) e executando uma revolução (`to: 0 360 0`) a cada 10 segundos (`dur: 10000`).
...
O movimento orbital será implementado pela rotação do sistema de coordenadas associado ao astro. Para isso, incluiremos o componente `animation` de rotação que agora rotacionará o astro não em torno do centro, mas em torno da posição dada pelo componente `position`
...
A @tbl-periodos contém os períodos de rotação e orbital dos três astros.
